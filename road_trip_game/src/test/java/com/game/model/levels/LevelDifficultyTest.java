...
class LevelDifficultyTest {
    ...
    void checkFieldsSanity() {
        //Max 6 fields. There is an implicit private static $VALUES field
        assertEquals(6, ownClass.getDeclaredFields().length);
        //Max 5 public fields
        assertEquals(5, Arrays.stream(ownClass.getDeclaredFields()).filter(ModifierSupport::isPublic).count());
        ...
        //Max 5 Enum values
        assertEquals(5, Arrays.stream(ownClass.getDeclaredFields()).filter(Field::isEnumConstant).count());
        //Max 6 static fields. There is an implicit private static $VALUES field
        assertEquals(6, Arrays.stream(ownClass.getDeclaredFields()).filter(ModifierSupport::isStatic).count());
